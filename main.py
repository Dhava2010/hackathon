...
# -------------------------------------------------
#  CAMERA: LET OPENCV PICK THE BEST BACKEND (GSTREAMER)
# -------------------------------------------------
def open_camera():
    # DO NOT use cv2.CAP_V4L2 — let OpenCV auto-select
    cap = cv2.VideoCapture(0)  # This will use GStreamer on Ubuntu
...
# -------------------------------------------------
#  READ FIRST FRAME (with long timeout)
# -------------------------------------------------
def read_first_frame(timeout=30):
    ...
    while time.time() - start < timeout:
        ...
        if ret and frame is not None:
            return frame
        print(f"  Still waiting... ({int(timeout - (time.time() - start))}s)")
        time.sleep(0.2)
    raise RuntimeError("Camera opened but no frames after 30s. Check cable/power.")
